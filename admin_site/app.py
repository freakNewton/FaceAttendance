...
conn = pymysql.connect(host="localhost",user="root",password="Maha@1997",db="login_info")
...
def login():
	...
	cursor = conn.cursor()
	result = cursor.execute("SELECT * from admin_login where  username=%s and password=%s",[user,paswd])
	if(result is 1):
